...
public interface DeviceRepository extends JpaRepository<Device, UUID> {
        ...
        /**
         * Lấy danh sách device đang ONLINE nhưng sắp bị đánh OFFLINE (chưa cập nhật dữ
         * liệu quá threshold).
         * JOIN FETCH farm để tránh LazyInitializationException trong @Scheduled context
         * (ngoài HTTP transaction).
         */
        @Query("""
        ...
        /**
         * Dùng cho watchdog safety auto-off: lấy actuator đang ON ở chế độ AUTO.
         */
        @Query("""
                        SELECT d FROM Device d
                        JOIN FETCH d.farm f
                        JOIN FETCH f.owner
                        JOIN FETCH d.model m
                        WHERE d.operatingMode = com.yoloFarm.api.enums.OperatingModeEnum.AUTO
                                AND COALESCE(d.isActive, false) = true
                                AND d.status = com.yoloFarm.api.enums.DeviceStatusEnum.ACTIVE
                                AND m.deviceType = com.yoloFarm.api.enums.DeviceTypeEnum.ACTUATOR
                        """)
        List<Device> findActiveAutoActuatorsWithFarmAndOwner();
}
